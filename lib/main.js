...
function uncurry( fcn, arity, thisArg ) {
	...
	var len;
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '1Y73c', fcn ) );
	}
	if ( arguments.length < 2 ) {
		...
		if ( !isPositiveInteger( len ) ) {
			throw new TypeError( format( '1Y7Az', len ) );
		}
	}
	...
	function uncurried() {
		...
		if ( len ) {
			if ( len > arguments.length ) {
				throw new Error( format( '1Y7Bs', len, arguments.length ) );
			}
			for ( i = 0; i < len; i++ ) {
				if ( isFunction( f ) ) {
					f = f.call( context, arguments[ i ] );
				} else {
					throw new Error( format( '1Y7Bv', len, i ) );
				}
			}
			return f;
		}
		for ( i = 0; i < arguments.length; i++ ) {
			if ( isFunction( f ) ) {
				...
				throw new Error( format( '1Y7Bu', arguments.length, i ) );
			}
		}
		return f;
	}
